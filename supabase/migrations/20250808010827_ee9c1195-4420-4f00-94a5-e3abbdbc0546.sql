...
do $$ begin
  if not exists (select 1 from pg_type where typname = 'polo_name') then
...
begin
  ...
  if cur_role = 'admin' then return true; end if;
  if req.user_id = cur_user_id then return true; end if;
  if cur_role = 'gestora' and req.polo = any(polos) then return true; end if;
  return false;
end;
$$;
...
-- Storage policies (drop/create specific names)
-- invoices bucket
drop policy if exists "Storage: public read invoices" on storage.objects;
drop policy if exists "Storage: users upload invoices" on storage.objects;
drop policy if exists "Storage: users update own invoices" on storage.objects;
drop policy if exists "Storage: users delete own invoices" on storage.objects;

create policy "Storage: public read invoices" on storage.objects
for select to public
using (bucket_id = 'invoices');

create policy "Storage: users upload invoices" on storage.objects
for insert to authenticated
with check (bucket_id = 'invoices' and auth.uid()::text = (storage.foldername(name))[1]);

create policy "Storage: users update own invoices" on storage.objects
for update to authenticated
using (bucket_id = 'invoices' and auth.uid()::text = (storage.foldername(name))[1])
with check (bucket_id = 'invoices' and auth.uid()::text = (storage.foldername(name))[1]);

create policy "Storage: users delete own invoices" on storage.objects
for delete to authenticated
using (bucket_id = 'invoices' and auth.uid()::text = (storage.foldername(name))[1]);

-- request-attachments bucket
drop policy if exists "Storage: public read request-attachments" on storage.objects;
drop policy if exists "Storage: users upload request-attachments" on storage.objects;
drop policy if exists "Storage: users update own request-attachments" on storage.objects;
drop policy if exists "Storage: users delete own request-attachments" on storage.objects;

create policy "Storage: public read request-attachments" on storage.objects
for select to public
using (bucket_id = 'request-attachments');

create policy "Storage: users upload request-attachments" on storage.objects
for insert to authenticated
with check (bucket_id = 'request-attachments' and auth.uid()::text = (storage.foldername(name))[1]);

create policy "Storage: users update own request-attachments" on storage.objects
for update to authenticated
using (bucket_id = 'request-attachments' and auth.uid()::text = (storage.foldername(name))[1])
with check (bucket_id = 'request-attachments' and auth.uid()::text = (storage.foldername(name))[1]);

create policy "Storage: users delete own request-attachments" on storage.objects
for delete to authenticated
using (bucket_id = 'request-attachments' and auth.uid()::text = (storage.foldername(name))[1]);
